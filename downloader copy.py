...
def print_banner():
    ...
    __  __        __           _ _     _                  
   |  \/  |      / _|         | | |   | |                 
   | \  / | __ _| |_ ___  _ __| | | __| | ___ _ __  _   _ 
   | |\/| |/ _` |  _/ _ \| '__| | |/ _` |/ _ \ '_ \| | | |
   | |  | | (_| | || (_) | |  | | | (_| |  __/ | | | |_| |
   |_|  |_|\__,_|_| \___/|_|  |_|_|\__,_|\___|_| |_|\__, |
                                                    __/ |
                                                   |___/ 
    """
    print(HEADER + banner)
    ...
    try:
        ...
        for f in formats:
            ...
            if f.get('vcodec') == 'none' and not audio_added:
                ...
                if f.get('acodec') != 'none' or not any(x.get('height') == height and x.get('acodec') != 'none' for x in formats):
                    filtered_formats.append(f)
        ...
        for f in filtered_formats[:25]: # Обмежимо для читабельності
            ...
            if f.get('vcodec') == 'none': note += " (audio only)"
            elif f.get('acodec') == 'none': note += " (video only)"
            print(f"{f.get('format_id', ''):<8} | {f.get('ext', ''):<10} | {f.get('resolution', 'audio only'):<20} | {note}")
